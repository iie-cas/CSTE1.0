...
攻击名称，攻击采用的方式为（主要有：return-to-libc，rop，cop，jop，fop，dop等）
...
// help 命令
输入“help”，直接按回车键，会显示 CSTE 当前版本号。以及 CSTE 框架支持的命令。

// help + 命令名称
例如输入 “help compile”，按回车键，会显示 compile 命令的相关帮助信息。

// 提示信息说明
通常提示信息都是以以下的形式给出，其中命令的一般格式都为 命令名称 +　命令参数，其中“=”后面的，“[]”内的内容为该命令可以使用的参数。

// compile
例如以下提示信息表示 compile 命令后可以使用的参数为“all、“rop”或者“uaf”。整体就是“compile all”、“compile rop”、“compile uaf”。其中“compile all”可以一次编译所有已有的样例。rop 和 uaf 分别为单个样例的样例名称。
compile [args]
[args] = ['all', 'rop', 'uaf']
...
// aslr
aslr命令可以用于配置 ASLR 防御机制，当然该防御机制也可以在配置文件中进行配置。这里的主要用途是可以用该命令查看 ASLR 的状态。特别提示：修改 ASLR 的状态需要 ROOT 权限。 
aslr [args]
[args] = ['status', 'on', 'off']
...
// show
show 命令主要用于列出当前例子的名称。后续会继续扩展（配置文件中，有若干配置项用于扩展该功能（type、name、target）），目前只支持一个参数“samples”。
show [args]
[args] = ['samples']
...
### 统计工作的完善
统计输出的结果不整洁，在解决了上述问题之后，才会考虑解决这个问题。
